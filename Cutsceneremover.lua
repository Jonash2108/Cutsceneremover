if not menu.is_trusted_mode_enabled() then
    menu.notify("Trusted Mode not enabled!", red)
end
local shit = menu.add_feature("Disable Shit", "parent", 0).id

menu.add_feature("Disable Shit", "toggle", shit, function(f)
if f.on then
    native.call(0xD79185689F8FD5DF, false)
    native.call(0x9641A9FF718E9C5E, false)
    native.call(0xFF300C7649724A0B, player.player_id(), false)
else
    native.call(0xD79185689F8FD5DF, true)
    native.call(0x9641A9FF718E9C5E, true)
    native.call(0xFF300C7649724A0B, player.player_id(), true)
end
end)

menu.add_feature("Disable Shit Loop", "toggle", shit, function(f)
    while f.on do
